          ...
          {Object.keys(cropResults.newPriceMap).length > 0 ? (
            Object.keys(cropResults.newPriceMap).map(cropName => (
              <div key={cropName} className="p-4 border rounded-lg shadow-md bg-white">
                ...
                <p className={Number(getChangeRate(cropName)) >= 0 ? 'text-blue-600' : 'text-red-600'}> {cropResults.originPriceMap[cropName].toLocaleString()}원 {'->'} {cropResults.newPriceMap[cropName].toLocaleString()}원</p>
